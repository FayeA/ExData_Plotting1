...
## SECTION B - CREATE THE PLOT
   ...
   png("plot4.png", bg="transparent") # open png graphic device. Set background to transparent.

   par(mfcol=c(2,2))  # make 4 plots fit
...
## PLOT 3 - Voltage
      ...
      plot(pcData$Time, pcData$Voltage,
           pch=".",  # set character to a dot so that it would blend in with the line
           ylab="Voltage", # set y label
           xlab="datetime", # remove "Index" in x axis,
           bg="transparent" # set background colour to transparent
      )
      points(pcData$Voltage,col=rgb(1, 1, 1, 1), pch=".", ) # set colour and transparency to "1" so that the "." would not show in the plot
      
      lines(pcData$Time, pcData$Voltage) # add lines to the scatter plot so it would look like what's provided in the assignment
...
## PLOT 4 - Global Reactive Power
      ...
      plot(pcData$Time, pcData$Global_reactive_power,
           pch=".",  # set character to a dot so that it would blend in with the line
           ylab="Global_reactive_power", # set y label
           xlab="datetime", # remove "Index" in x axis,
           bg="transparent" # set background colour to transparent
      )
      points(pcData$Global_reactive_power,col=rgb(1, 1, 1, 1), pch=".", ) # set colour and transparency to "1" so that the "." would not show in the plot
      
      lines(pcData$Time, pcData$Global_reactive_power) # add lines to the scatter plot so it would look like what's provided in the assignment
